document.addEventListener("DOMContentLoaded", () => {
  ...
  const sparklingBeveragesOutputEl = document.getElementById(
    "sparkling-beverages-output"
  );
  ...
  function getAPIUrl() {
    ...
    if (idx !== -1) {
      return (
        parts.slice(0, idx).join("/") + "/backend/api/production/dashboard.php"
      );
    }
    return "/backend/api/production/dashboard.php";
  }
  ...
  if (
    !totalProductionEl ||
    !bottledWaterOutputEl ||
    !sparklingBeveragesOutputEl ||
    !lowStockItemsEl ||
    !trendsChartCanvas ||
    !inventoryTbody
  ) {
  ...
  const renderKpiCards = (dailyOutput, stockLevels) => {
    ...
    bottledWaterOutputEl.textContent = (
      dailyOutput.bottled_water || 0
    ).toLocaleString();
    sparklingBeveragesOutputEl.textContent = (
      dailyOutput.sparkling_beverages || 0
    ).toLocaleString();
  ...
  const renderInventorySummary = (stockLevels) => {
    ...
    if (stockLevels.length === 0) {
      inventoryTbody.innerHTML =
        '<tr><td colspan="3" class="text-center text-gray-500 p-4">No inventory data.</td></tr>';
      return;
    }
    ...
    stockLevels.forEach((item) => {
      ...
      const row = `
                <tr>
                    <td class="p-2 border-b text-sm">${escapeHtml(
                      item.product_name
                    )}</td>
  ...
  const renderTrendsChart = (trendsData) => {
    if (!trendsData || !trendsChartCanvas) {
      console.warn("Trends data or canvas not available");
      return;
    }

    // Check if a chart already exists and destroy it
    if (window.productionTrendsChartInstance) {
      window.productionTrendsChartInstance.destroy();
    }

    const ctx = trendsChartCanvas.getContext("2d");
    window.productionTrendsChartInstance = new Chart(ctx, {
      type: "line",
      data: {
